...
def generate_simple_config():
    ...
    try:
        # 创建简化配置
        simple_config, encoded_key = create_simple_config()
        
        print(f"✅ 找到API密钥，环境: {os.getenv('NODE_ENV', 'development')}")
        
        # 验证配置
        if not validate_config(simple_config):
            return False
    ...
    # 生成配置哈希（用于验证）
    config_hash = generate_config_hash(simple_config)
...
def main():
    ...
    try:
        ...
        # 生成配置文件
        success = generate_simple_config()
